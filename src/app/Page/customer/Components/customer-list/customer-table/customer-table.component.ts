...
export class CustomerTableComponent implements OnInit,OnDestroy {
  ...
  constructor(private _coreService: FacadService,private dialog: MatDialog) { 
    this.isLoading$ = this._coreService.customer.isLoading$;
  }
  ...
  delete(row:Customer){
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result != undefined && result) {
        const sb=this._coreService.customer.delete(row.id).subscribe(result=>{
          if(result?.isSuccess){
  ...
  getData(){
    const sb=this._coreService.customer.items$.subscribe(data=>{
